...
因為我想要在 clean project 時同步刪除特定的目錄。不像 Build 有明確的 Pre-build Event 或 Post-build Event 可以掛上額外的工作，客製 Clean 行為似乎沒有那麼簡單，至少 Visual Studio 並沒有 UI 可以直接操作。

## 研究 .vcxproj

研究了一下 vcxproj 的文件格式，我發現整個 C++ 專案的編譯流程，都是由 `Microsoft.Cpp.targets` 這個文件定義的。每個 .vcxproj 透過 `<Import>` tag 將之引入，確立整個專案的建構流程。
...
OK，所以其實 Visual C++ 在 Clean 的流程中已經保留了 BeforeCppClean 和 AfterCppClean 兩個事件，分別在 Clean 之前和之後觸發，我只要複寫這兩個事件就行了。

## 複寫 BeforeCppClean

現在我要重新定義 Target，作法是寫一個自己的 .target 文件，然後再透過 `<Import>` 標籤引入 vcxproj。
實際上要做的工作就寫在 Target 標籤裡。
